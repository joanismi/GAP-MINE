...
disease_conservative_modules <- read.csv('../python/data/processed/string_disgenet_modules.csv')
disease_modules <- disease_conservative_modules[disease_conservative_modules$increase<0.4,]
...
#------------------Normal modules--------------------------------------
disease_raw_scores <- matrix(0, length(disease_modules[['process']]), length(V(ppi_graph)$name))
rownames(disease_raw_scores) <- disease_modules[['process']]
colnames(disease_raw_scores) <- V(ppi_graph)$name

for(i in 1:length(disease_modules[['process']])){
  module_scores <- disgenet_labels[,i]
  names(module_scores) <- rownames(disgenet_labels)
  seleted_scores <- disgenet_labels[,i]
  names(seleted_scores) <- rownames(disgenet_labels)
  df_diff <- diffuse_grid(graph=ppi_graph,
                          K = kernel_scores,
                          scores = seleted_scores,
                          grid_param = expand.grid(method = c('raw')),
...
write.csv(t(disease_raw_scores),"../python/data/processed/metrics/disease_raw_string.csv")
...
#------------------Normal modules--------------------------------------
disease_conservative_raw_scores <- matrix(0,length(disease_conservative_modules[['process']]) , length(V(ppi_graph)$name))
rownames(disease_conservative_raw_scores) <- disease_conservative_modules[['process']]
colnames(disease_conservative_raw_scores) <- V(ppi_graph)$name


for(i in 1:length(disease_conservative_modules[['process']])){
  module_scores <- disgenet_conservative_labels[,i]
  names(module_scores) <- rownames(disgenet_conservative_labels)
  seleted_scores <- disgenet_conservative_labels[,i]
  names(seleted_scores) <- rownames(disgenet_conservative_labels)
  df_diff <- diffuse_grid(graph=ppi_graph,
                          K = kernel_scores,
                          scores = seleted_scores,
                          grid_param = expand.grid(method = c('raw')),
...
write.csv(t(disease_conservative_raw_scores),"../python/data/processed/metrics/disease_conservative_raw_string.csv")
