...
def paper_boxplot_comparison(df, filename, colors=['#59C3C3', '#FDA96D'], showlegend=True):
    ...
    fig.update_layout(
        ...
        width=2000,
        title_x=0.5,
        legend=dict(font=dict(size=60, color='black'),
                    yanchor="bottom", y=0.075, xanchor="right", x=0.99),
...
def paper_boxplot_method_comparison(df, filename, colors=['#FDA96D', '#59C3C3', '#FDA96D', '#59C3C3'], showlegend=True):
    traces = []
    for i, md in enumerate(df.metric.unique()):
        df_plot = df[(df.metric == md)]
        df_plot.sort_values(by='quality_metric', inplace=True)
        trace = go.Box(
            x=df_plot.quality_metric,
            y=df_plot.score,
            boxpoints='outliers',
            name=md,
            fillcolor=colors[i],
